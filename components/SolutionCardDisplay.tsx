...
const CardContent: React.FC<{ card: SolutionCard }> = ({ card }) => {
    return (
        <div className="bg-neutral-800/80 rounded-lg p-3 w-full max-w-xs group transition-all duration-300 border border-neutral-700/60 hover:border-brand-700/50 hover:bg-neutral-800">
             <div className="flex items-start gap-3">
                ...
                <div>
                    <h4 className="text-sm font-semibold text-neutral-100">{card.title}</h4>
...
export const SolutionCardDisplay: React.FC<SolutionCardDisplayProps> = ({ card }) => {
    if (card.link) {
        return (
